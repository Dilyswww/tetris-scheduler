...
export function ItemForm({ date, onAdd, onClose }: { date: string; onAdd: (item: CalendarItem) => Promise<string | null>; onClose: () => void }) {
  ...
  return (
    <Dialog title="Add to your day" onClose={onClose}>
      <form onSubmit={submit} className="item-form">
        ...
        <p className="form-hint">{kind === "fixed" ? "Fixed events stay at the time you choose." : "We'll find a continuous gap before your deadline. You can pin the task after it's scheduled."}</p>
